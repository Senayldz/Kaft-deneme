...
function UpdateProduct() {
  ...
  useEffect(() => {
    axios.get(`https://kaft-deneme-3y3m.vercel.app/products/${id}`)
      .then(res => {
        setProduct(res.data);
      })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`https://kaft-deneme-3y3m.vercel.app/products/${id}`, formData, {
        headers: {
  ...
  const onDrop = useCallback((acceptedFiles) => {
    ...
    axios.post('https://kaft-deneme-3y3m.vercel.app/products/upload', formData, {
      headers: {
        'Content-Type': 'multipart/form-data'
      }
    })
    ...
    <div className="container">
      <div className="form-container">
        <form onSubmit={handleSubmit}>
          ...
          {product.image && (
            <div>
              <img src={`https://kaft-deneme-3y3m.vercel.app/${product.image}`} alt="Product" style={{ width: '100px', height: '100px' }} />
            </div>
          )}
